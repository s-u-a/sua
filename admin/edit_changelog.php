...
*/
	...
	$old_version = $version = explode('.', Config::get_version(), 3);
...
<form action="edit_changelog.php" method="post">
	<fieldset>
		...
		<div><button type="submit"<?=l::accesskey_attr(_("Speichern&[admin/edit_changelog.php|1]"))?>><?=h(_("Speichern&[admin/edit_changelog.php|1]"))?></button></div>
	</fieldset>
</form>
<form action="edit_changelog.php" method="post">
	<ul>
		<li><input type="text" name="add" value="" /> <button type="submit"<?=l::accesskey_attr(_("Hinzufügen&[admin/edit_changelog.php|1]"))?>><?=h(_("Hinzufügen&[admin/edit_changelog.php|1]"))?></button></li>
<?php
	$changelog = '';
	if(is_file(global_setting("DB_CHANGELOG")) && is_readable(global_setting("DB_CHANGELOG")))
	...
	foreach($changelog as $i=>$log)
	{
		...
		echo " [<a href=\"edit_changelog.php?delete=".htmlspecialchars(urlencode(count($changelog)-$i))."\"".l::accesskey_attr(_("Löschen&[admin/edit_changelog.php|1]")).">".h(_("Löschen&[admin/edit_changelog.php|1]"))."</a>]</li>\n";
	}
?>
	</ul>
</form>
