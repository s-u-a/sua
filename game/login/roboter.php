...
<?php
	...
	{
?>
	<div class="button"><button type="submit" tabindex="<?=$tabindex++?>"<?=l::accesskey_attr(_("In A&uftrag geben[login/roboter.php|1]"))?>><?=l::h(_("In A&uftrag geben[login/roboter.php|1]"))?></button></div>
<?php
	}
?>
</form>
...
?>
	<li class="<?=htmlspecialchars($first[0])?> active<?=(count($building_roboter) <= 0) ? ' last' : ''?>"><strong><?=l::h(_("[item_".$first[0]."]"))?> <span class="restbauzeit" id="restbauzeit-<?=$i++?>"><?=htmlspecialchars(F::format_ftime($first[1], $me))?></span></strong></li>
<?php
		if(count($building_roboter) > 0)
		{
			$keys = array_keys($building_roboter);
...
<form action="<?=htmlspecialchars(global_setting("USE_PROTOCOL").'://'.$_SERVER['HTTP_HOST'].global_setting("h_root").'/login/roboter.php?'.global_setting("URL_SUFFIX"))?>" method="post" class="alle-abbrechen">
	...
	<div><input type="password" name="cancel-all-roboter" /><input type="submit" value="<?=l::h(_("Alle abbrechen&[login/roboter.php|1]"), false)?>"<?=l::accesskey_attr(_("Alle abbrechen&[login/roboter.php|1]"))?> /></div>
</form>
<?php
	}
