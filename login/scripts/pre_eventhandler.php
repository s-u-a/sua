<?php
	function pre_eventhandler()
	{
		global $user_array;
		global $items;
		global $ges_prod;

		# Hier werden Events dem Eventhandler vorweggenommen, damit niemand warten muss
					...
					if(!$flotte[7])
					{
						...
						if($flotte[2] == 1)
						{
							...
							if($planet_info[1])
							{
								...
								messages::new_message(array($_SESSION['username']=>5), '', 'Planet '.$flotte[3][0].' bereits besetzt', 'Ihre Flotte erreicht den Planeten '.$flotte[3][0].' und will mit der Besiedelung anfangen. Jedoch ist der Planet nicht mehr frei und Ihre Flotte tritt den Rückweg an.');
							}
							elseif(count($user_array['planets']) >= 15)
							{
								...
								messages::new_message(array($_SESSION['username']=>5), '', 'Besiedelung von '.$flotte[3][0].' fehlgeschlagen', 'Ihre Flotte erreicht den Planeten '.$flotte[3][0]." und will mit der Besiedelung anfangen. Als Sie jedoch Ihren Zentralcomputer um Best\xc3\xa4tigung für die Besiedelung bittet, kommt dieser durcheinander, da Sie schon so viele Planeten haben und er nicht so viele gleichzeitig kontrollieren kann, und schickt in Panik Ihrer Flotte das Signal zum R\xc3\xbcckflug.");
							}
							else
							{
								...
								universe::set_planet_info($koords[0], $koords[1], $koords[2], $planet_info[0], $_SESSION['username'], $new_planet_array['name']);
								...
								if((count($flotte[0]) == 1 && isset($flotte[0]['S6']) && $flotte[0]['S6'] <= 1) || count($flotte[0]) < 1)
								{
									...
									if($user_array['receive'][5][0])
									{
										...
										messages::new_message(array($_SESSION['username']=>5), '', 'Besiedelung von '.$flotte[3][1], $message_text);
									}
						...
						elseif($flotte[2] == 2)
						{
							...
							messages::new_message(array($_SESSION['username']=>4), '', 'Abbau auf '.$flotte[3][0], $nachrichten_text, true);
						...
						elseif($flotte[2] == 3)
						{
							...
							$start_own = ($start_info[1] == $_SESSION['username']);

							$target_pos = explode(':', $flotte[3][1]);
							$target_info = universe::get_planet_info($target_pos[0], $target_pos[1], $target_pos[2]);
							$target_own = ($target_info[1] == $_SESSION['username']);
						...
						elseif($flotte[2] == 4 || $flotte[2] == 6)
						{
							...
							else
							{
								...
								if($flotte[2] == 4)
								{
									...
									if($start_own && $target_own)
									{
										...
										if($user_array['receive'][3][0])
										{
											...
											messages::new_message(array($_SESSION['username']=>3), '', 'Ankunft Ihres Transportes auf '.$flotte[3][0], $message_text);
										}
									}
									else
									{
								...
								else
								{
									...
									# Nachricht versenden
									if($user_array['receive'][3][0])
										messages::new_message(array($_SESSION['username']=>3), '', 'Stationierung auf '.$flotte[3][1], $message_text);
								}
					...
					if($flotte[7] && $flotte[1][1] <= time())
					{
						...
						if($planet_index !== false)
						{
							...
							if($user_array['receive'][$types_message_types[$flotte[2]]][1])
							{
								...
								$start_pos = explode(':', $flotte[3][0]);
								$start_info = universe::get_planet_info($start_pos[0], $start_pos[1], $start_pos[2]);
								if($start_info[1] == $_SESSION['username'])
								...
								messages::new_message(array($_SESSION['username'] => $types_message_types[$flotte[2]]), '', "R\xc3\xbcckkehr von ".$flotte[3][0], $message_text);
