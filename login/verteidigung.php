...
?>
	<li class="<?=htmlspecialchars($first[0])?> active<?=(count($building_verteidigung) <= 0) ? ' last' : ''?>"><strong><?=h(_("[item_".$first[0]."]"))?> <span class="restbauzeit" id="restbauzeit-<?=$i++?>"><?=htmlspecialchars(format_ftime($first[1], $me))?></span></strong></li>
<?php
		if(count($building_verteidigung) > 0)
		{
			$keys = array_keys($building_verteidigung);
