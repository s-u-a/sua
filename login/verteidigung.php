...
<?php
	...
	{
?>
	<div class="button"><button type="submit" tabindex="<?=$tabindex++?>"<?=l::accesskey_attr(_("In A&uftrag geben[login/verteidigung.php|1]"))?>><?=h(_("In A&uftrag geben[login/verteidigung.php|1]"))?></button></div>
<?php
	}
?>
</form>
...
?>
	<li class="<?=htmlspecialchars($first[0])?> active<?=(count($building_verteidigung) <= 0) ? ' last' : ''?>"><strong><?=h(_("[item_".$first[0]."]"))?> <span class="restbauzeit" id="restbauzeit-<?=$i++?>"><?=htmlspecialchars(F::format_ftime($first[1], $me))?></span></strong></li>
<?php
		if(count($building_verteidigung) > 0)
		{
			$keys = array_keys($building_verteidigung);
