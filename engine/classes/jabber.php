...
class Jabber
{
	...
	function SendAuth()
	{
		if(isset($_SERVER['REMOTE_ADDR'])) $auth_addr = $_SERVER['REMOTE_ADDR'];
		else $auth_addr = '217.160.132.126';
		$this->auth_id	= "auth_" . md5(time() . $auth_addr);
	...
	function _check_connected()
	{
		...
		if (is_array($incoming_array) && isset($incoming_array['stream:stream']))
		{
	...
	function _xml_depth($vals, &$i)
	{
		...
		while (++$i < count($vals))
		{
			switch ($vals[$i]['type'])
			{
				...
				case 'complete':
					$tagname = $vals[$i]['tag'];
					if(!isset($children[$tagname])) $children[$tagname] = array();
