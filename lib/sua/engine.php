...
*/
	...
	*/
	...
	global_setting("start_mtime", microtime(true));
	...
	global_setting("s_root", realpath(substr($__FILE__, 0, -strlen($this_filename))));
	...
	elseif(isset($_SERVER["DOCUMENT_ROOT"]) && substr(global_setting("s_root"), strlen($tmp = realpath($_SERVER["DOCUMENT_ROOT"]))) == $tmp)
	...
	global_setting("h_root", "".substr(global_setting("s_root"), strlen($document_root)));

	# Locale eintragen
	bindtextdomain("sua", global_setting("s_root")."/locale");
	bind_textdomain_codeset("sua", "utf-8");
	textdomain("sua");

	# PEAR einbinden
	if(is_dir(global_setting("s_root")."/pear"))
		set_include_path(".:".global_setting("s_root")."/pear/:".get_include_path());
	...
	*/
	...
	function __autoload($classname)
	{
		...
		include_once($fname);
		if(is_subclass_of($classname, "StaticInit"))
			$classname::init();
	}
	...
	//if(isset($_SERVER["HTTP_ACCEPT"]) && strpos($_SERVER["HTTP_ACCEPT"], "application/xhtml+xml") !== false)
		//global_setting("CONTENT_TYPE", "application/xhtml+xml; charset=UTF-8");
	//else
		global_setting("CONTENT_TYPE", "text/html; charset=UTF-8");
	if(isset($_SERVER["HTTP_HOST"]))
		header("Content-type: ".global_setting("CONTENT_TYPE"));

	// Script-Filename herausfinden
	if(!isset($_SERVER["SCRIPT_FILENAME"]) && substr($_SERVER["PHP_SELF"], 0, strlen(global_setting("h_root"))) == $_SERVER["PHP_SELF"])
		$_SERVER["SCRIPT_FILENAME"] = global_setting("s_root").substr($_SERVER["PHP_SELF"], strlen(global_setting("h_root")));
	...
	global_setting("VERSION", $version);
